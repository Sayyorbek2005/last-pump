...
export default function CodeGenerator({ lang = "uz" }) {
  ...
  const fetchBatches = useCallback(async () => {
    try {
      const { data: rawUsedData, error: usedError } = await supabase
        .from("used_codes")
        .select(`
          id,
          code_id,
      ...
      setUsedCodes(formattedUsedCodes);

      const usedCodeIdsSet = new Set((rawUsedData || []).map(uc => uc.code_id));
      ...
      const activeAndPaused = allData.filter(c => c.status !== "used" && !usedCodeIdsSet.has(c.id));
      const groups = {};
